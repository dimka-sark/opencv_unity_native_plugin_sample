...
// add the calculated number of fingers to image m->src_frame
void HandGesture::addNumberToImg(MyImage *m){
    ...
    for(int i=0;i<numbers2Display.size();i++){
        rectangle(m->src_frame,Point(xPos,yPos),Point(xPos+offset,yPos+offset),numberColor, 2);
        putText(m->src_frame, intToString(numbers2Display[i]),Point(xPos+7,yPos+offset-3),fontFace,fontSize,numberColor);
        xPos+=40;
        if(xPos>(m->src_frame.cols-m->src_frame.cols/3.2)){
            yPos+=40;
            xPos=10;
        }
    }
...
void HandGesture::getFingerNumber(MyImage *m){
    ...
    if(bRect.height > m->src_frame.rows/2 && nrNoFinger>12 && isHand ){
...
void HandGesture::drawFingerTips(MyImage *m){
    ...
    for(int i=0;i<fingerTips.size();i++){
        p=fingerTips[i];
        putText(m->src_frame,intToString(i),p-Point(0,30),fontFace, 1.2f,Scalar(200,200,200),2);
        circle( m->src_frame,p,   5, Scalar(100,255,100), 4 );
    }
}
