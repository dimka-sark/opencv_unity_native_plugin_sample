...
extern "C"
{
void JNICALL Java_com_example_dimkasark_nikitkabuild2_MainActivity_salt(JNIEnv *env, jobject instance,
                                                                           jlong matAddrGray,
                                                                           jint nbrElem) {
...
extern "C" void DLL_EXPORT Close()
{
    my_image->video_capture.release();
    destroyAllWindows();
    roi.clear();
...
extern "C" int DLL_EXPORT  Init(int video_source, int& outCameraWidth, int& outCameraHeight)
{
    ...
    outCameraWidth = static_cast<int>(my_image->video_capture.get(CAP_PROP_FRAME_WIDTH));
    outCameraHeight = static_cast<int>(my_image->video_capture.get(CAP_PROP_FRAME_HEIGHT));
    return 0;
}

extern "C" void DLL_EXPORT  SetHandsColor()
{
#ifndef ANDROID_BUILD
    /*--------------------DEBUG ZONE START--------------------*/
    namedWindow("img1", CV_WINDOW_KEEPRATIO);
...
extern "C" void DLL_EXPORT GetCoordinates(int& x, int& y)
{
    my_image->video_capture >> my_image->src_frame;
    flip(my_image->src_frame, my_image->src_frame, 1);
    pyrDown(my_image->src_frame, my_image->srcLR);
...
extern "C" int DLL_EXPORT GetFingersCount()
{
    if (!hand_gesture.isHand)
    {
        return -1;
